...
export async function discoverTracks(server: SensorHubServer, withCredentials: boolean): Promise<IObservable[]> {

    let observables: IObservable[] = [];

    await discover(server, withCredentials, "tracks")
        .then(discoveryData => {
            ...
            for (let result of resultSet) {
                ...
                let trackData = fetchFromObject(result, 'tracks');
                ...
                for (let track of trackData) {
                    ...
                    trackDataSource = new SweApi(physicalSystem.name + "-track-dataSource", {
                        ...
                        collection: `/datastreams/${track.dataStreamId}/observations`,
                ...
                let pointMarkerLayer: PointMarkerLayer = new PointMarkerLayer({
                    getLabel: {
                        ...
                        handler: function (rec: any) {
                            let label: string = findInObject(rec, 'transponder_id');
                            if (label != null) {
                                label = 'JFFT: ' + label;
                            } else {
                                label = 'JFFT: Unknown Track';
                            }
                            return label;
                        }
                    },
                    getMarkerId: {
                        ...
                        dataSourceIds: [trackDataSource.getId()],
                        handler: function (rec: any) {
                            let id: string = findInObject(rec, 'transponder_id');
                            if (id == null) {
